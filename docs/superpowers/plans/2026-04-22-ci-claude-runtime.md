...
- [ ] `GH_PAT` is provisioned as an **organization secret** in the `glitchwerks` org (Settings → Secrets and variables → Actions → New organization secret) with `repo:read` scope on `glitchwerks/claude-configs`, and this repo (`glitchwerks/github-actions`) is on its repository-access list. Org-level provisioning is preferred over repo-level so other repos that adopt the runtime later can consume the same secret without re-issuing it. Proof: screenshot or text listing of the secret's presence (not its value) at the org level, plus repo access confirmation, attached as a comment to the Phase 0 sub-issue.
- [ ] `glitchwerks/claude-configs` has at least one `ci-v<semver>` tag containing the artifacts the manifest imports (skills `git`, `python`; agents `ops`, `inquisitor`, `debugger`, `code-writer`; `CLAUDE.md`; `standards/software-standards.md`). Proof: tag name + commit SHA recorded in the Phase 0 sub-issue.
...
- [ ] **3.1** Author `runtime/overlays/review/Dockerfile`. `FROM ghcr.io/glitchwerks/claude-runtime-base@sha256:${BASE_DIGEST}` (digest passed as `--build-arg BASE_DIGEST=<sha>` per §6.2 STAGE 3). Steps: install `pr-review-toolkit` plugin (P1 from marketplace clone at pinned SHA), copy `inquisitor` agent from private import, copy review overlay CLAUDE.md. `chmod -R a+rX /opt/claude/.claude/`. Commit.
...
- [ ] **3.3** Author `runtime/overlays/review/expected.yaml` literally matching §10.2 example: `must_contain.agents: [inquisitor, comment-analyzer, pr-test-analyzer, silent-failure-hunter, type-design-analyzer, code-reviewer, code-simplifier]`, `must_contain.skills: [git]`, `must_contain.plugins: [context7, github, microsoft-docs, typescript-lsp, security-guidance, pr-review-toolkit]`, `must_not_contain.agents: [code-writer, debugger]`, `must_not_contain.plugins: [skill-creator]`. Commit.
- [ ] **3.4** Author `runtime/overlays/fix/Dockerfile` mirroring the review Dockerfile but importing `debugger`, `code-writer` from private per §5.1 `overlays.fix.imports_from_private`. No `pr-review-toolkit`. Commit.
- [ ] **3.5** Author `runtime/overlays/fix/CLAUDE.md`. Scope: write/fix/refactor code on the consumer's branch. Documents the `--read-only` contract: when invoked with `--read-only`, the overlay must produce NO commits (diagnosis-only). Explicitly: "Never skip git hooks — see §9.2: if pre-commit rejects, let the commit fail; do not use `--no-verify`." Must name the `debugger`, `code-writer` agents as available. Commit.
- [ ] **3.6** Author `runtime/overlays/fix/expected.yaml`. `must_contain.agents: [debugger, code-writer]`, inherited base plugins, `must_not_contain.agents: [inquisitor, code-reviewer, comment-analyzer, pr-test-analyzer]`, `must_not_contain.plugins: [pr-review-toolkit]`. Commit.
...
- [ ] **3.9** Author `runtime/overlays/explain/expected.yaml`. `must_contain.plugins` = the base plugin set (inherited). `must_not_contain.agents: [code-writer, debugger, inquisitor, code-reviewer]`. `must_not_contain.plugins: [pr-review-toolkit]`. Commit.
